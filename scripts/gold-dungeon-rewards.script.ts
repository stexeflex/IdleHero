...
function ComputeGoldRewardForStage(dungeonId: string, stage: number): number | null {
  ...
  const baseGoldReward = Math.round(dungeon.Rewards.GoldBase * stageFactor);

  if (stage === dungeon.StagesMax) {
    return Math.round(baseGoldReward * REWARDS_CONFIG.GOLD.COMPLETION_MULTIPLIER);
  }

  if (dungeon.MidStages.includes(stage)) {
    return Math.round(baseGoldReward * REWARDS_CONFIG.GOLD.MID_BOSS_MULTIPLIER);
  }
...
function BuildRows(maxStage: number): DungeonStageRewardRow[] {
  ...
  for (let stage = 1; stage <= maxStage; stage++) {
    ...
    for (const dungeonId of dungeonIds) {
      const reward = ComputeGoldRewardForStage(dungeonId, stage);
